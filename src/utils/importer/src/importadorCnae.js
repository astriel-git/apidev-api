...
async function copyCSVToTable (client, csvFilePath, progressBar) {
  ...
  const transformStream = new Transform({
    ...
    transform (chunk, encoding, callback) {
      ...
      // Replace any remaining semicolons with commas in the description field
      if (chunk[1]) {
        chunk[1] = chunk[1].replace(/;/g, ',')
      }

      // Convert the chunk to UTF-8 encoding
...
async function main () {
  ...
  try {
    ...
    const csvFilePath = '../../unzipper/unzipped/Cnaes/F.K03200$Z.D40713.CNAECSV'
